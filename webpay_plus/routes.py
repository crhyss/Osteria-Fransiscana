...
def webpay_plus_create(request):
    ...
    return render(request, 'webpay/send-pay.html', {'response': response, 'amount': amount}) 
...
def commitpay(request):
    ...
    if TBK_TOKEN is None and TBK_ID_SESSION is None  and TBK_ORDEN_COMPRA is None and token is not None:
        ...
        if status == 'AUTHORIZED' and response_code == 0:
            ...
            return render(request, 'carrito/confirmacion-retiro.html', {'transaction_detail': transaction_detail,})
        else:
